...
input_weights_SE = 'UNETSTEAM_RETGA_E30_MB15_LR01_DF98-1_SD_MO98_SSIM95MAE5_nores'; %If not blank, path to the U-network that must be loaded to copy its weights
input_weights_STEAM = 'UNETSTEAM_RETGA_E30_MB15_LR01_DF98-1_SD_MO98_SSIM95MAE5_nores'; %If not blank, path to the U-network that must be loaded to copy its weights
...
lgraph = connectLayers(lgraph, 'split1','STEAM_conv1');
lgraph = connectLayers(lgraph, 'split2','SE_conv1');
...
lgraph = connectLayers(lgraph,'STEAM_pool4','conca_SEQ/in1');
lgraph = connectLayers(lgraph,'SE_pool4','conca_SEQ/in2');
lgraph = connectLayers(lgraph,'STEAM_relu8','conca1/in2');
lgraph = connectLayers(lgraph,'STEAM_relu6','conca2/in2');
lgraph = connectLayers(lgraph,'STEAM_relu4','conca3/in2');
lgraph = connectLayers(lgraph,'STEAM_relu2','conca4/in2');

%plot(lgraph);
clearvars -except img_size lgraph dataset perc ntest augmentation;
...
[TR_NO_DS,TR_DE_DS,VA_NO_DS,VA_DE_DS]= generate_ynet1_datastore(dataset,perc,ntest,augmentation);
...
[trained_ynet_STEAM,info] = trainNetwork(training_datastore,lgraph,options);
