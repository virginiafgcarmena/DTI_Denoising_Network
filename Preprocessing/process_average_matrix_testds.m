function [subjects, av_images450, av_images150] = process_average_matrix_testds(subjects,type)

%Input: subjects (structure coming from readaset_Virginia) that are mean
%for TEST PURPOSE. The difference between the original average_matrix
%program is that B0 images and images for which we haven't got enough
%averages are preserved. In addition, to have at least one b = 150 per
%(subject, phase, sequence and direction), we set the average index of
%b=150 images to 1 for STEAM and 2 for SE. Last, it leaves the data from
%the DICOM header in the denoised images to test them. 
...
for p=1:length(subjects) %We loop accross all patients
    ...
    for ph=1:num_phases %We loop accross the number of sequences
        ...
        for seq=1:num_sequences %We loop accross the number of sequences
            ...
            %STEP 1: Note down the number of 450, 150 images.
                ...
                for j=1:size(diffd,3) %Loop accoss averages
                  if ~isempty(diffd{1,i,j})
                    ...
                    switch diffd{1,i,j}.b_pres %450, 150 or b0?
                        ...
                        otherwise 
                            if (avmat(i,j)==0) %We only empty b0 if average_matrix points to 0
                                subjects{p}.(name_phases{ph}).(name_sequences{seq}).diffusiondata{1,i,j} = [];
                            end
                     end
                  end
                    ...
                    if length(n_450)>= averaged_index
                       ...
                       if (counter_450 == 0) %If we are on one first item of one average
                            if counter_image==1
                                ...
                                av_images450{p}.(name_phases{ph}).(name_sequences{seq}).diffusiondata{counter_image,i}.info = diffd_corr{1,i,n_450(a)}.info;
                            else
                ...
                %n_images = floor(length(n_450)/averaged_index); 
                tag_last_image = 0;
                if seq==1 averaged_index =2; end
                if seq==2 averaged_index =1; end
                    ...
                    if length(n_150)>= averaged_index
                       ...
                       if (counter_150 == 0) %If we are on one first item of one average
                            if counter_image==1
                                ...
                                av_images150{p}.(name_phases{ph}).(name_sequences{seq}).diffusiondata{counter_image,i}.info = diffd_corr{1,i,n_150(a)}.info;
                            else
                                %New image implies new row!
