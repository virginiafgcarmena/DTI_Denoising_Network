classdef Splitting < nnet.internal.cnn.layer.Layer
    % Splitting Layer
    ...
    properties(SetAccess = private, Dependent)
        %OutputNames   The names are determined by the number of inputs
        OutputName
    end
    ...
    methods
        function this = Splitting(name, SplittingAxis, numOutputs)
            % Concatenation   Constructor for this layer
            this.Name = name;
            this.SplittingAxis = SplittingAxis;
            this.NumOutputs = numOutputs;
            
            % Concatenation layer needs X but not Z for the backward pass
        ...
        function outputSeqLen = forwardPropagateSequenceLength(~, inputSeqLen, ~)
            ...
            % Valid input sequence lengths should either be all ones, or
            % all sequence tokens
            if iAllInputSeqLensAreEqual( inputSeqLen, inputSeqLen{1} )
                outputSeqLen = inputSeqLen(1);
            else
                % The following error will be caught and re-thrown by the
                % Network Analyzer
                iThrowInconsistentSeqLenError();
            end
        end
...
function tf = iAllInputSeqLensAreEqual( cellArrayOfSeqLen, seqLen )
tf = all( cellfun(@(s)isequal(s, seqLen), cellArrayOfSeqLen) );
end

function iThrowInconsistentSeqLenError()
error( message('nnet_cnn:internal:cnn:layer:Concatenation:InvalidSeqLen') );
end