function [TR_NO_DS,TR_DE_DS,VA_NO_DS,VA_DE_DS]= generate_unet1_datastore(unified_dataset,perc,ntest)
...
if exist('training_imgs')==7 %If the folder exists
    ...
    training_done = 0; mkdir('training_imgs'); mkdir('training_imgs/denoised_imgs'); %LINUX
end
if exist('val_imgs')==7 %If the folder exists
    validation_done = 1;
else
    %validation_done = 0;  mkdir('val_imgs'); mkdir('val_imgs\denoised_imgs');
    validation_done = 0;  mkdir('val_imgs'); mkdir('val_imgs/denoised_imgs'); %LINUX
end
...
elseif  ntest==0
    ...
    test_done = 0; mkdir('test_imgs'); mkdir('test_imgs/denoised_imgs'); %LINUX
end
...
%Put the data in folders to later generate the datastore
    if ~training_done
       ...
       training_data = augmentation_unet(training_data,12,3);
       ...
       dataset2folders(training_data,fullfile(pwd,'training_imgs/'));  %LINUX      
    end
    if ~validation_done
...
%       dataset2folders(validation_data,fullfile(pwd,'val_imgs\'));
        dataset2folders(validation_data,fullfile(pwd,'val_imgs/')); %LINUX
    end
    if ~test_done
...
%       dataset2folders(test_data,fullfile(pwd,'test_imgs\'));
        dataset2folders(test_data,fullfile(pwd,'test_imgs/'));  %LINUX     
    end
