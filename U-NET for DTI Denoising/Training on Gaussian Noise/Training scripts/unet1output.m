classdef unet1output < nnet.layer.RegressionLayer
    ...
    methods   
        ...
        function loss = forwardLoss(layer, Y, T)
            ...
            loss = 0.3*loss_SSIM+0.7*loss_MAE;
        ...
        function dLdY = backwardLoss(layer, Y, T)
            ...
            individual_filter = fspecial('gaussian',layer.size_f,70); %Gaussian Filter 
            gaussian_f = repmat(individual_filter, [1,1,1,size(Y,4)]); %We repeat the filter along MB direction
            
            dldx = (single(zeros(size(gaussian_f))));
            dcdx = (single(zeros(size(gaussian_f))));
            dLdX = (single(zeros(size(gaussian_f))));
            ...
            dLdY = 0.3*dLdY_SSIM+0.7*dLdY_MAE;
        end      
    end
